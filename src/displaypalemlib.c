...
EMSTATUS PAL_SpiInit (void)
{
  ...
  PAL_SPI_USART_UNIT->ROUTE = (USART_ROUTE_CLKPEN | USART_ROUTE_TXPEN | USART_ROUTE_RXPEN | PAL_SPI_USART_LOCATION);
...
uint8_t USART_SpiTransfer1(USART_TypeDef *usart, uint8_t data)
{
...
uint8_t PAL_SpiTransfer(uint8_t data)
{
#if 0
	  while (len>0)
	  {
	  ...
	  return (uint8_t)(PAL_SPI_USART_UNIT->RXDATA);
#endif
	  return USART_SpiTransfer1(PAL_SPI_USART_UNIT, data);
}

uint8_t PAL_SpiReceive(USART_TypeDef *usart)
{
	USART_Tx( PAL_SPI_USART_UNIT, 0);
	return USART_Rx(PAL_SPI_USART_UNIT);
}
                            ...
                            unsigned int   platformSpecific)
{
  ...
  case palGpioModePushPull:
    ...
    break;
  case 1:
      emGpioMode = gpioModeInput;
      break;
  case 2:
	emGpioMode = gpioModeInputPull;
	break;
  default:
 ...
 * @return  EMSTATUS code of the operation.
 ******************************************************************************/
unsigned int PAL_GpioPinOutGet(unsigned int port, unsigned int pin)
{
  EMSTATUS status = PAL_EMSTATUS_OK;

  return GPIO_PinOutGet((GPIO_Port_TypeDef) port, pin);
}
